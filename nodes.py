...
class Node(nodes_pb2_grpc.RaftManagerServicer):
    ...
    def ServeClient(self, request, context):
        if self.state!="LEADER":
            return nodes_pb2.ServeClientReply(Success=False,LeaderID=self.leader_id,Data="failed requuest because it is not the leader or the lease is expired")
        if self.leader_id is None :
            return nodes_pb2.ServeClientReply(Success=False,LeaderID=None,Data="failed requuest because the leader does no exist")
        myReq=request.Request.split()
        if myReq[0]=="GET":
            for item in reversed(self.log):
                myData=item.split()
                if myData[1]==myReq[1]:
                    return nodes_pb2.ServeClientReply(Success=True,LeaderID=self.leader_id,Data=f"Success value is {myData[2]}")
            return nodes_pb2.ServeClientReply(Success=False,LeaderID=self.leader_id,Data=f"failed no corresponding value ")
        print("sending ack to all nodes")
        ack=1
        self.log.append(f"{request.Request} {self.term}")
    ...
    # Check if the received term is less than the current term
        ...
        # self.log = request.entries
        self.update_metadata(self.commit_index,self.term,self.voted_for)
        self.prevLogIndex=len(self.log)-1
    ...
    def leader_lease_expired(self):
        print(f"Leader {self.leader_id} lease renewal failed.")
        text = f"Leader {self.leader_id} lease renewal failed. Stepping down..."
        self.append_dump(text)
        self.state = "FOLLOWER"
        self.leader_id = None
    ...
    def send_heartbeats(self):
        if self.state == "LEADER":
            ...
            if ack>=(len(self.nodes)+1)/2:
                print("recevied majority ack of hearbeats")
                self.restart_leader_lease_timer()
